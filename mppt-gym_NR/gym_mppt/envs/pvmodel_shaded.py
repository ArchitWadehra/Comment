...
class Panel(object):
    ...
    def __init__(self):
        ...
        self.Rs = 8e-3 # Series resistance 8mOhm
    ...
    def calc_pv(self, G, T, vx, SH):
        ...
        Vt = self.k * Tcell * self.A / self.q 
        diff = float('inf')
        while diff > 10e-4:
            # fv = I - (Iph*self.Np - Irs*self.Np*(np.exp(((vx/self.Ns) + ((I*Rs)/self.Np))/Vt) - 1) - ((((vx*self.Np)/self.Ns) + (I*Rs))/Rsh))
            fv = I - (Iph*self.Np - Irs*self.Np*(np.exp(((vx/(self.Ns*SH)) + ((I*self.Rs)/self.Np))/Vt) - 1))
            # fdv = 1 + (Rs/Rsh) + ((np.exp(((vx/self.Ns) + ((I*Rs)/self.Np))/Vt)*Irs*Rs)/(Vt))
            fdv = 1 + ((np.exp(((vx/(self.Ns*SH)) + ((I*self.Rs)/self.Np))/Vt)*Irs*self.Rs)/(Vt))
            # In = I - (fv./fdv)
            In = I - fv/fdv
            diff = abs(abs(In) - abs(I))
            I = In
